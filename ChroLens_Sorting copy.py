...
class AutoMoveApp:
    def __init__(self, root):
        ...
        self.dest_entries = []
        self.kind_var = tb.StringVar(value="5")  # 預設顯示5個欄位
        ...
        tb.Button(zero_frame, text="移入路徑", command=lambda: self.select_dest_folder(self.entry_all_path)).pack(side=LEFT, padx=5)
    ...
    def move_files(self):
        ...
        if all_path:
            remain_files = [f for f in all_files if f not in moved_files and os.path.isfile(os.path.join(src, f))]
